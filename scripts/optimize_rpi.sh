#!/bin/bash
# Optymalizacja Raspberry Pi dla rtasp

set -e  # Zatrzymanie przy błędzie

echo "===== Optymalizacja Raspberry Pi dla rtasp ====="
...
if ! grep -q "# rtasp optimizations" /boot/config.txt; then
  echo "" >> /boot/config.txt
  echo "# rtasp optimizations" >> /boot/config.txt
...
if ! grep -q "vm.swappiness" /etc/sysctl.conf; then
  ...
  echo "# rtasp optimizations" >> /etc/sysctl.conf
